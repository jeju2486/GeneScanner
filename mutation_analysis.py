...
def parse_arguments():
    ...
    parser.add_argument('-a', '--alignment', required=True, help='FASTA alignment file')
    parser.add_argument('-t', '--type', required=True, choices=['p', 'n', 'both'],
                        help='Alignment type: "p"=protein, "n"=nucleotide, "both"=both')
    parser.add_argument('-f', '--frame', default=1, type=int, help='Reading frame for nucleotide alignment')
...
def analyze_nucleotide_alignment(sequences, aln_length, frame=1):
    ...
    for pos in range(start_pos, aln_length):
        ...
        # Counters
        syn_count = nonsyn_count = ins_count = del_count = stop_count = 0
        mutations = {}

        # Check each sample for mutations
        for sid in samples:
            ...
            if wt_nt == '-' and sample_nt != '-':
                mtype = 'Insertion'; ins_count += 1
            elif wt_nt != '-' and sample_nt == '-':
                mtype = 'Deletion'; del_count += 1
            else:
                sample_codon_raw = sequences[sid][start_pos + codon_index*3 : start_pos + codon_index*3 + 3]
                sample_codon = sample_codon_raw.replace('-', '')
                sample_aa = translate_codon(sample_codon) if len(sample_codon) == 3 else ''

                if sample_aa == '*':
                    mtype = 'Stop Codon'; stop_count += 1
                elif sample_aa == ref_aa and sample_aa != '':
                    mtype = 'Synonymous'; syn_count += 1
                elif sample_aa != ref_aa and sample_aa != '' and ref_aa != '':
                    mtype = 'Non-synonymous'; nonsyn_count += 1
                else:
                    mtype = 'Unknown'
...
def analyze_protein_alignment(sequences, aln_length):
    ...
    for pos in range(aln_length):
        ...
        ins_count = del_count = stop_count = sub_count = 0
        mutations = {}
        ...
        for sid in samples:
            ...
            if wt_aa == '-' and sample_aa != '-':
                ...
                if sample_aa == '*':
                    ...
                    mtype = 'Substitution'; sub_count += 1
...
def summarize_data(df, analysis_type):
    ...
    if analysis_type == 'n':
        ...
        for c in ['Synonymous Mutations', 'Non-synonymous Mutations', 'Insertions', 'Deletions', 'Stop Codons', 'Total Mutations']:
        ...
        for c in ['Insertions', 'Deletions', 'Stop Codons', 'Substitutions', 'Total Mutations']:
            df[c] = pd.to_numeric(df[c])
        summary.update({
...
def write_output(output_file, nuc_data, prot_data, sequences, prot_sequences, nuc_matrix, prot_matrix):
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        # Nucleotide sheets
        if nuc_data:
            df_nuc = write_analysis_sheet(writer, nuc_data, 'Nucleotide Analysis', sequences[list(sequences.keys())[0]])
        ...
        # Protein sheets
        if prot_data:
            df_prot = write_analysis_sheet(writer, prot_data, 'Protein Analysis', prot_sequences[list(prot_sequences.keys())[0]])
...
def main():
    ...
    nuc_data = prot_data = None
    nuc_matrix = prot_matrix = None
    prot_sequences = None

    # Perform analyses
    if aln_type == 'n':
        nuc_data, nuc_matrix = analyze_nucleotide_alignment(sequences, aln_length, frame=args.frame)
    elif aln_type == 'p':
        ...
        prot_data, prot_matrix = analyze_protein_alignment(prot_sequences, len(prot_sequences[list(prot_sequences.keys())[0]]))

    # Default output if none provided
    output_file = args.output if args.output else f"{aln_type_full}_mutation_analysis.xlsx"
    write_output(output_file, nuc_data, prot_data, sequences, prot_sequences or {}, nuc_matrix, prot_matrix)
